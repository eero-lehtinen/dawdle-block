...
export class GeneralOptions {
	...
	static async create(browserStorage: BrowserStorage): Promise<GeneralOptions> {
		...
		if (res.isErr()) return new GeneralOptions(browserStorage)
	...
	assignData(generalOptionsData?: GeneralOptionsData): void {
		if (generalOptionsData === undefined) this._data = createDefaultGeneralOptionsData()
		else this._data = generalOptionsData
	}
