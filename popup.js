...
function loadAllBlocksets() {
    ...
    for (id of blocksetIds) {
        ...
        var time = $("<span>", { class: "blocksiteTime"}).text(msToTime(blocksetDatas[id].timeAllowed - blocksetDatas[id].timeElapsed));
...
function update() {
    for(id of blocksetIds) {
        if (blocksetTimes[id] != undefined) {
            blocksetTimes[id].text(msToTime(blocksetDatas[id].timeAllowed - blocksetDatas[id].timeElapsed));
        } 
    }
}
